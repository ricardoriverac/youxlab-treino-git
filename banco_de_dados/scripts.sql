...
insert into municipio (nome , iduf) values ('Teste 1' , 2)
...
-- Campos Default 
alter table pedido alter data_pedido set default current_date;
alter table pedido alter valor set default 0;
...
insert into pedido_produto (idpedido , idproduto) values (16,1);


delete from produto where idproduto = 19
alter table produto alter column valor set default 0;
insert into produto (idfornecedor , nome) values (2 , 'Computador');
select * from produto
...
-- Exercicios
-- Apagando indice
drop index idx_pdd_data
create index idx_pdd_data on pedido (data_pedido)
create index idx_prd_nome on produto (nome)


select * from pedido_produto order by idproduto asc;
select * from pedido order by idpedido asc;
select * from produto;
